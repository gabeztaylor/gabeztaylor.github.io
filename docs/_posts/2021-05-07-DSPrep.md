...
This is an active document that I am actively updating in preparation for my full-time interview this summer. More topics to come. 
...
How to grow a regression tree:
  1. For j in splitting variables

     a. For s in split points
        
          Calculate average of the target in each split region and compute the sum of squares 

  2. Choose the best pair j, s that minimize the sum of squares
